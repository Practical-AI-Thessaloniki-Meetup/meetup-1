from core.test_cases import filter_based_on_test_case
import pandas as pd
...
def run_experiment(model: DecisionTreeRegressor, parameters: Any):
    ...
    # Select noise mode
    df = filter_based_on_test_case(df, noise)

    # Create experiment.
    run_name = create_run_name(experiment_name, noise, mode, scale)
    ...
    with mlflow.start_run(experiment_id=experiment_id, run_name=run_name):
        ...
        # Saving the model.
        dump(
            model_results.best_estimator_,
            f"./results/{experiment_name}/{run_name}/model.joblib",
        )
        mlflow.sklearn.log_model(model_results.best_estimator_, "model")


def create_run_name(prefix: str, noise: int, mode: str, scale: str) -> str:
    """
    ...
    :return: The run name.
    """
    return f"{prefix}_noise_{noise}_mode_{mode}_scale_{scale}_{int(time.time())}"
